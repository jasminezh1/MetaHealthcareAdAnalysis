...
def countExc(filename):
    adCount = 0
    excCount = 0
    with open(filename, 'r') as csv_file:
        csv_reader = csv.reader(csv_file)
        for text in csv_reader:
            exclamations = text[0].count('!')
            if exclamations>0: 
                excCount += 1
            adCount += 1

    return adCount, excCount
...
for filename in filenames:
    ...
    adCount, excCount = countExc(filename)

    csv_file_path = 'excFinal.csv'
    new_line = [keyword] + [adCount] + [excCount]
